...
const TABLE = {
  ...
  [STATES.S7A]: { security_review_complete: t(STATES.S7B, ["run_patch_planning"]), security_review_failed: t(STATES.S14, ["return_to_builder_security_review", "require_postmortem"]), design_gap_found: t(STATES.S2, ["return_to_aristotle", "require_postmortem"]) },
  [STATES.S7B]: { patch_plan_complete: t(STATES.S7C, ["run_patch_task_planning"]), patch_plan_failed: t(STATES.S14, ["return_to_patch_planning", "require_postmortem"]) },
...
function builderConstraint() { return { constraint: "Builder is not allowed to build the whole design as a lump.", required_behavior: ["Feature Branch Workflow", "create or initialize the GitHub repository", "do not create parent and child refs in the same Git namespace", "use branch pattern feature/<feature-slug>", "use branch pattern subfeature/<feature-slug>--<sub-feature-path-slug>", "use branch pattern task/<feature-slug>--<sub-feature-path-slug>--<task-slug>", "use branch pattern patch/<feature-slug>--<sub-feature-path-slug>--<patch-id>", "do not nest Git worktrees inside other Git worktrees", "use worktree pattern ../worktrees/<repo-slug>/feature--<feature-slug>/", "use worktree pattern ../worktrees/<repo-slug>/subfeature--<feature-slug>--<sub-feature-path-slug>/", "plan one task slice", "implement one task slice", "run mapped Bacon validation", "check mapped Hoare correctness obligations", "check mapped Epictetus operational obligations", "confirm Diogenes cuts were not reintroduced", "document before next task", "plan one patch task", "implement one patch task", "document before next patch", "never batch patches"] }; }
