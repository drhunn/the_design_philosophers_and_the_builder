...
const REQUIRED_SECTIONS = ["git", "task", "patch", "validation", "documentation", "remaining_work"];
...
function parseValue(raw) {
  ...
  if (v.startsWith("[") && v.endsWith("]")) {
    return v.slice(1, -1).split(",").map(s => s.trim()).filter(Boolean).map(s => s.replace(/^"|"$/g, ""));
  }
  return v.replace(/^"|"$/g, "");
}
...
function parseTomlLoose(text) {
  ...
  for (const line of String(text || "").split(/\r?\n/)) {
    ...
    if (section) out[section][key] = value; else out[key] = value;
  }
  return out;
}

function subset(touched, allowed) {
  if (!Array.isArray(touched) || touched.length === 0) return true;
  return Array.isArray(allowed) && allowed.length > 0 && touched.every(f => allowed.includes(f));
}
...
function validateHandoffObject(handoff, guard = "task_documentation_updated") {
  ...
  const git = handoff.git || {}, task = handoff.task || {}, patch = handoff.patch || {}, validation = handoff.validation || {}, docs = handoff.documentation || {}, remaining = handoff.remaining_work || {};
  ...
  if (["task_slice_plan_complete", "task_documentation_updated", "all_task_slices_complete", "no_remaining_task_slices"].includes(guard)) {
    for (const k of ["task_id", "feature_slug", "task_slug", "purpose"]) if (!present(task[k])) errors.push(`[task].${k} is required`);
    if (!subset(task.touched_files || [], task.allowed_files || [])) errors.push("[task].touched_files must be a subset of [task].allowed_files");
  }
  if (["task_documentation_updated", "all_task_slices_complete", "no_remaining_task_slices", "patch_task_documentation_updated", "all_patch_tasks_complete", "no_remaining_patch_tasks"].includes(guard)) {
    for (const k of ["bacon_passed", "hoare_passed", "epictetus_passed", "diogenes_passed", "tests_passed"]) if (validation[k] !== true) errors.push(`[validation].${k} must be true`);
  }
  if (["task_documentation_updated", "all_task_slices_complete", "no_remaining_task_slices"].includes(guard)) {
    if (docs.task_documentation_updated !== true) errors.push("[documentation].task_documentation_updated must be true");
    if (!present(docs.task_documentation_path)) errors.push("[documentation].task_documentation_path is required");
  }
  if (["all_task_slices_complete", "no_remaining_task_slices"].includes(guard)) {
    if (remaining.no_remaining_task_slices !== true) errors.push("[remaining_work].no_remaining_task_slices must be true");
    if (Array.isArray(remaining.remaining_task_slices) && remaining.remaining_task_slices.length !== 0) errors.push("[remaining_work].remaining_task_slices must be empty");
  }
  if (["patch_task_plan_complete", "patch_task_documentation_updated", "all_patch_tasks_complete", "no_remaining_patch_tasks"].includes(guard)) {
    for (const k of ["patch_task_id", "patch_id", "kind", "risk_or_defect", "affected_branch", "affected_worktree_path", "merge_target"]) if (!present(patch[k])) errors.push(`[patch].${k} is required`);
    if (!present(patch.merge_path)) errors.push("[patch].merge_path is required");
    if (!subset(patch.touched_files || [], patch.allowed_files || [])) errors.push("[patch].touched_files must be a subset of [patch].allowed_files");
  }
  if (["patch_task_documentation_updated", "all_patch_tasks_complete", "no_remaining_patch_tasks"].includes(guard)) {
    if (docs.patch_documentation_updated !== true) errors.push("[documentation].patch_documentation_updated must be true");
    if (!present(docs.patch_documentation_path)) errors.push("[documentation].patch_documentation_path is required");
  }
  if (["all_patch_tasks_complete", "no_remaining_patch_tasks"].includes(guard)) {
    if (remaining.no_remaining_patch_tasks !== true) errors.push("[remaining_work].no_remaining_patch_tasks must be true");
    if (Array.isArray(remaining.remaining_patch_tasks) && remaining.remaining_patch_tasks.length !== 0) errors.push("[remaining_work].remaining_patch_tasks must be empty");
  }
  return errors;
}

function artifactFromContext(context = {}) {
...
function resolveAction(action) {
  if (Object.prototype.hasOwnProperty.call(ACTION_AGENT_FILES, action)) {
    ...
    const relativePath = `agents/${agentFile}`;
    const fullPath = path.join(__dirname, relativePath);
    return { action, kind: "agent", agent_file: agentFile, relative_path: relativePath, path: fullPath, exists: fs.existsSync(fullPath) };
  }
...
function loadAction(action) {
  ...
  if (!resolved.exists) throw new Error(`agent file not found for action ${action}: ${resolved.relative_path}`);
  return { ...resolved, content: fs.readFileSync(resolved.path, "utf8") };
}
...
function dispatch(state, event, context = {}) {
  ...
  if (transition.guard) {
    if (PROOF_GUARDS.has(transition.guard)) {
      const artifact = artifactFromContext(context);
      if (!artifact) return { ok: false, error: `Required proof handoff missing for guard: ${transition.guard}` };
      const errors = validateHandoffObject(artifact, transition.guard);
      if (errors.length) return { ok: false, error: `handoff validation failed for guard ${transition.guard}`, validation_errors: errors };
    } else if (context[transition.guard] !== true) {
      return { ok: false, error: `Required context flag not satisfied: ${transition.guard}`, required_context_flag: transition.guard };
    }
  }
...
function describe() {
  return { name: "The Design Philosophers and the Builder", purpose: "Bounded Mealy workflow with proof-carrying TOML handoffs, PRD Markdown from Plato, one-task slices, one-patch patch tasks, and fixed action-to-agent dispatch.", operations: ["describe", "available_events", "dispatch", "dispatch_and_load", "resolve_actions", "load_actions", "happy_path", "handoff_template", "validate_handoff", "routing_guidance", "builder_constraint"] };
}

function routingGuidance(userInput = "") {
  return { user_input: userInput, guidance: ["If it changes the real problem, route to Socrates.", "If it changes PRD/product form, route to Plato.", "If it changes structure, route to Aristotle.", "If it changes evidence or validation, route to Bacon.", "If it changes invariants or correctness, route to Hoare.", "If it changes failure behavior or operational tolerance, route to Epictetus.", "If it changes repository setup, feature worktree workflow, task slicing, or patch task implementation, route to Builder."], note: "Guidance only. Classify against the current bounded artifact before dispatching a state-machine event." };
}

function builderConstraint() {
  return { constraint: "Builder is not allowed to build the whole design as a lump.", required_behavior: ["Feature Branch Workflow", "create or initialize the GitHub repository", "do not create parent and child refs in the same Git namespace", "use branch pattern subfeature/<feature-slug>--<sub-feature-path-slug>", "do not nest Git worktrees inside other Git worktrees", "run mapped Bacon validation", "check mapped Hoare correctness obligations", "check mapped Epictetus operational obligations", "confirm Diogenes cuts were not reintroduced", "document before next task", "document before next patch", "never batch patches"] };
}
...
module.exports.runtime = { handler: async function ({ operation, state, event, context_json, user_input, artifact_toml, guard, action, actions_json }) {
  try {
    const op = operation || "describe";
    const context = parseContext(context_json);
    if (artifact_toml) context.artifact_toml = artifact_toml;
    ...
    if (op === "dispatch_and_load") {
      const result = dispatch(state, event, context);
      return JSON.stringify({ dispatch: result, loaded_actions: result.ok ? loadActions(result.actions) : [] }, null, 2);
    }
    if (op === "resolve_actions") return JSON.stringify({ actions: resolveActions(parseActions(actions_json, action)) }, null, 2);
    if (op === "load_actions") return JSON.stringify({ actions: loadActions(parseActions(actions_json, action)) }, null, 2);
    if (op === "happy_path") return JSON.stringify({ events: happyPath() }, null, 2);
    if (op === "handoff_template") return handoffTemplate();
    if (op === "validate_handoff") {
      const artifact = artifact_toml ? parseTomlLoose(artifact_toml) : artifactFromContext(context);
      const errors = artifact ? validateHandoffObject(artifact, guard || "task_documentation_updated") : ["artifact_toml is required"];
      return JSON.stringify({ ok: errors.length === 0, guard: guard || "task_documentation_updated", errors }, null, 2);
    }
